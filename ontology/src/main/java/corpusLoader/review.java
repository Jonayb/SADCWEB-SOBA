...
public class review {
	...
	private Map<String, String> all_terms_tag = new HashMap<String, String>(); // change to hashmap
	private HashSet<String> all_terms = new HashSet<>(); 
	...
	public Map<String, String> stanford_pipeline_tagger(StanfordCoreNLP pipeline, String pattern) {
		...
		for(int f = 0; f <doc.sentences().size(); f++) {
			for (int d = 0; d < doc.sentences().get(f).tokens().size(); d++) {
				...
				if (tag.contains("VB") == true|| tag.contains("JJ") == true || tag.contains("NN") == true || tag.contains("RB") == true){
					// Create a Pattern object
					Pattern r = Pattern.compile(pattern, Pattern.CASE_INSENSITIVE);
					// Now create matcher object.
					Matcher m = r.matcher(word.lemma());
					if (!m.find() && !word.originalText().equals("")) {
						all_terms_tag.put(word.lemma().toLowerCase(), tag);
					}
				}

			}
		}
		return all_terms_tag;
	...
	{ // MaxentTagger
		tagger = new MaxentTagger("C:\\Users\\Ruben\\git\\Heracles\\stanford-postagger-2018-10-16\\models\\english-bidirectional-distsim.tagger"); 
		for (int i = 0; i <listOfSentences.size(); i ++) { 
			String tagged = tagger.tagString(listOfSentences.get(i)); pos_tags.add(tagged); 
		} 
		return pos_tags; 
	}
	...
	public HashSet<String> get_adj_verb_noun() { 
		for( int j = 0; j<pos_tags.size(); j++) { 
			...
			while(regexMatcher.find()) { 
				String lemma = new Sentence(regexMatcher.group(1).split("_")[0]).lemma(0); //
